#업데이트 2018.11.9. 
library(httr) # HTTP통신
library(rvest) # HTML요소
library(stringr)
library(curl)
library(xml2)
library(sjmisc) #is_empty 함수 활용
#setwd("E:/(프로젝트)/2017/기본과제/텍스트데이터")
g=c()
d=c()
time=c()
time1=c()
...
for(year in 2017:2019){
  ...
  for(month in 1:12){
    ...
    for(day in 1:31){
      ...
      k2 = na.omit(k2) #NA 값이 없는 값을 삭제하는 함수
  ...
  for(i in g){
    ...
    for (url in url_list){
      ...
      headline = html_nodes(htxt1, css='div.article_info') %>% html_nodes('h3#articleTitle') %>%
        html_text()
      # 간간이 enetertain 예능 기사가 나올 때. html구조가 달라짐.
      ...
      print(date)
      # 기사입력, 최종수정. 2개의 날짜가 있음. dataframe만들때 row갯수 불일치하는 결과. 하나만 선택하기로.
      dday = c(dday, date[1])
